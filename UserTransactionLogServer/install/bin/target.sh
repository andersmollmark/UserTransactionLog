...
TEMP=`getopt -o e --long embedded -n 'target.sh' -- "$@"`
...
if [ $embedded = true ] ; then
    ...
    echo "Add services to runlevel vms2"
    addServiceVms mysql
    rc-update -u
...
#mysql --user=root --password=delavalpwd -e "GRANT SUPER ON *.* TO logAdmin@localhost IDENTIFIED BY 'admin'"
    ...
    echo "Finish installing embedded utls"
else
...
endInstallation
/usr/bin/logger -t install Installation finished!
